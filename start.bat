...
if not exist "venv\Scripts\activate.bat" (
    echo [WARN] Virtual environment tidak ditemukan. Membuat venv...
    python -m venv venv
...
start "API Server" cmd /k "cd /d %~dp0 && call venv\Scripts\activate.bat && set PYTHONPATH=%~dp0 && python -m uvicorn api.main:app --reload --host 0.0.0.0 --port 8000"
